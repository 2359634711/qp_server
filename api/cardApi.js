...
function _select_all_card() {
    return new Promise((resolve) => {
        query('SELECT * FROM `card_list`', (err, response) => {
            resolve({
                err: !!err,
...
function _select_card_from_typeid(typeid) {
    return new Promise((resolve) => {
        query('SELECT * FROM `card_list` where `typeid` = ?', [typeid], (err, response) => {
            resolve({
                err: !!err,
                data: err ? err : response
            })
...
function _insert_card(obj) {
    let { typeid, img, title, content, info, custom_data, input_line } = obj;
    return new Promise((resolve) => {
        query("INSERT INTO `card_list` (`typeid`, `img`, `title`, `content`, `info`, `custom_data`, `input_list`) VALUES (?, ?, ?, ?, ?, ?, ?)",
            [typeid || null, img || null, title || null, content || null, info || null, custom_data || null, input_line || null], (err, response) => {
                resolve({
                    err: !!err,
                    data: err ? err : response
                })
...
function _update_card(obj) {
    let { typeid, img, title, content, info, custom_data, input_line, id } = obj;
    return new Promise((resolve) => {
        query("UPDATE `card_list` SET `typeid`=?, `img`=?, `title`=?, `content`=?, `info`=?, `custom_data`=?, `input_list`=? WHERE (`id`=?)",
            [typeid || null, img || null, title || null, content || null, info || null, custom_data || null, input_line || null, id], (err, response) => {
                resolve({
                    err: !!err,
                    data: err ? err : response
                })
